import numpy as np
from models import Motor, Rocket, Environment, FlightState
...
def generate_random_trial(motor, dry_mass, drag_coefficient, cross_sec_area, sea_level_density):
    dry_mass_sample = rng.normal(loc=dry_mass, scale=DRY_MASS_STD*dry_mass)
    k_sample = rng.normal(loc=1.0, scale=MOTOR_SCALE_STD)
    drag_coefficient_sample = rng.normal(loc=drag_coefficient, scale=DRAG_COEFFICIENT_STD*drag_coefficient)
    ...
    sampled_rocket = Rocket(
        ...
        drag_coefficient=drag_coefficient_sample,
...
def run_monte_carlo(motor, dry_mass, drag_coefficient, cross_sec_area, sea_level_density, N_TRIALS):
    apogee_list = []
    max_velocity_list = []
    for i in range(N_TRIALS):
        sampled_rocket, sampled_environment = generate_random_trial(motor, dry_mass, drag_coefficient, cross_sec_area, sea_level_density)
